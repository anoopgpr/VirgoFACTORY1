...
var DirecctLogin = "https://taghash.co/qrcodevirgolam/";
...
$(document).ready(function(event){
	...
	document.addEventListener("backbutton", function(e)
	{
	 ...
	 if(localStorage.getItem("usernamemloyal_Virgo")!=null && localStorage.getItem("usernamemloyal_Virgo")!='' && localStorage.getItem("userpassloyal_Virgo")!=null && localStorage.getItem("userpassloyal_Virgo")!='')
	 {
		$('#forgot_mobile').val(localStorage.getItem("usernamemloyal_Virgo"));
		$('#Password_login').val(localStorage.getItem("userpassloyal_Virgo"));
...
{          
	...
	// earn_Scanpage('V3-G-A-12345-123-072-1ALD3MTP2J');
...
{
	if(localStorage.getItem("virgoCategory")=='factory' ){
		getScannerDetails(bcode);
	}
	else if(localStorage.getItem("virgoCategory")=='retailer'){
		getScannerDetails_retailer(bcode);
	}else{
...
function logoutApp()
{
	localStorage.removeItem("usernamemloyal_Virgo");
	localStorage.removeItem("userpassloyal_Virgo");

	localStorage.removeItem("virgoCategory");
...
function receivedEvent(id) {
 ...
 var push = PushNotification.init({ "android": {"senderID": "193303184404"}});
...
function burnpoints_byqr(camount,userid)
{
	...
	var refbillno =  'virgolam'+Math.floor((Math.random() * 1000000000) + 1);
		...
		$.ajax({
		   ...
		   success: function(data, textStatus, xhr) {
			...
			if(JSON.parse(data).success==true)
			{
				...
				localStorage.setItem('usernamemloyal_Virgo',uname);
				localStorage.setItem('userpassloyal_Virgo',password);
				localStorage.setItem('virgoCategory',JSON.parse(data).role);
...
function rejectScannerPopup(){
	$("#scan-on-popup").css("display","none");
	$("#scan-on-popup-green").css("display","none");
	$("#scan-on-popup-red").css("display","none");
	
}	
function closeScannerPopup(){
	$("#scan-on-popup").css("display","none");
	// $("#scan-click-home").on("click", function () {
        $("#scan-on-popup").show();
    //   });
	
}
function getScannerDetails(scanCode)
{
	// $("#scan-click-home").on("click", function () {
        // $("#scan-on-popup").show();
    //   });
	$.ajax({
		// url: "http://localhost:8086/"+SERVER2+"api_DeactivateCoupon.asp",
		url: SERVER2+"api_DeactivateCoupon.asp",
		type: 'POST',
		timeout: 50000,
		dataType: 'html',
		data:{
		...
		// headers: {
		// 	"apiuid": "MLOYALAPI",
		// 	"apipswd": "M0e B!Q$T@2o19"
		//    },
		beforeSend: function (xhr) {

			 $("#wait").css("display","block");
			xhr.setRequestHeader('apiuid', 'MLOYALAPI');
			xhr.setRequestHeader('apipswd', 'M0B!Q$T@2o19');

		},
		complete: function(xhr, textStatus) {
			$("#wait").css("display","none");
			// console.log(SERVER2+"api_DeactivateCoupon.asp?"+"couponCode="+scanCode+"&apiuid=MLOYALAPI&apipswd=M0e B!Q$T@2o19")
		},
		success: function(data, textStatus, xhr) {
			
			 console.log("data:"+JSON.stringify(data));
			//  $('#scanner_value').val(JSON.stringify(data));
			// document.getElementById("scanner_value").innerHTML= JSON.stringify(data);
			//  $("#scan-on-popup").show();
			 if(data.length>=1)
			 {
				if(data=='DeActive'){
						$("#okButton").css("display","none");
						$(".scan-inner-wrap").css("background-color","#faf605");
						$("#scanner_value").css("color","#000");
					document.getElementById("scanner_value").innerHTML= 'Coupon Already Deactivated';
					setTimeout( function() {
						rejectScannerPopup();
					 }, 3000);
				}else{
					// document.getElementById("scanner_value").innerHTML= data;  
					$("#okButton").css("display","none");
					$(".scan-inner-wrap").css("background-color","#4043a9");
					$("#scanner_value").css("color","#ffffff");
					document.getElementById("scanner_value").innerHTML= data;
					setTimeout( function() {
						rejectScannerPopup();
					 }, 3000);
...
function getScannerDetails_retailer(scanCode)
{
	// $("#scan-click-home").on("click", function () {
        // $("#scan-on-popup").show();
    //   });
	$.ajax({
		// url: "http://localhost:8086/"+SERVER2+"api_getCouponStatus.asp",
		url: SERVER2+"api_getCouponStatus.asp",
		type: 'POST',
		timeout: 50000,
		dataType: 'html',
		data:{
		...
		success: function(data, textStatus, xhr) {
			...
			//  $("#scan-on-popup").show();
			 ...
			 console.log(data.indexOf("is active"));
				if(data.indexOf("is active")>=1)
				{
					$("#scan-on-popup-green").show();
					document.getElementById("scanner_value_green").innerHTML= data;
					setTimeout( function() {
						rejectScannerPopup();
					 }, 3000);
				}
				else if(data.indexOf("is already scanned")>=1)
				{
					$("#scan-on-popup-red").show();
					document.getElementById("scanner_value_red").innerHTML= data;
					setTimeout( function() {
						rejectScannerPopup();
					 }, 3000);
				}
